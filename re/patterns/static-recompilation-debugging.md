...
## a repeating byte pattern in a crash address can be compiler code padding, not uninitialized data

A crash address like 0xCCCC0014 (or any suspiciously regular repeating-byte pattern) is easy to jump to "uninitialized heap memory" or "an MSVC debug-fill marker" as an explanation, since 0xCC is exactly the fill byte MSVC/CRT debug allocators use. Before committing to that theory, read the real bytes at the computed base address in a proper disassembler. In one investigation the base pointer causing an 0xCCCC0014 read turned out to be a genuine, valid-looking code address - the tail end of a completely unrelated function's epilogue - immediately followed by a long run of 0xCC bytes that were ordinary compiler inter-function alignment padding, not heap fill and not application data at all. The actual bug was a mid-function jump target getting called as if it were an independent function, inheriting garbage register/stack state from whatever unrelated call site happened to reach it - the 0xCC bytes were just where that garbage pointer happened to land, not evidence of an uninitialized-memory bug in their own right. Ruling out "this is code, not data" first (one Ghidra byte read) would have shortcut a much longer chain of hypotheses about missing constructors and grown-array capacity.

## verify a decode-only auto-seeded function actually has a CALL cross-reference before trusting it

An auto-fixer that seeds missed functions purely from "this address decodes as clean, plausible x86" cannot distinguish a real function entry from a mid-function jump target or jump-table case that merely happens to also decode cleanly starting from that byte offset - both look identical to a bare instruction decoder with no cross-reference analysis. A concrete tell, checkable without Ghidra: if the seeded function's own lifted code reads a register as its very first real instruction with no assignment to that register anywhere earlier in its own body, it is relying on register state some caller or jump established - which is normal for a fastcall-style function argument, but should be treated as a warning sign for a large, structurally complex auto-seeded candidate. When Ghidra is available, get_xrefs_to the address directly: a real function has at least one CALL reference (or is a documented entry point); an address reached only by JMP or a jump table's DATA reference, with zero CALL references, is a mid-function label that was mistakenly promoted to an independent function. Calling it as one runs it with essentially random inherited register/stack state from whatever unrelated call site the recompiler's reconstructed graph happened to route through, producing crashes whose root cause looks nothing like the actual bug (missing constructor, corrupted array, wrong vtable) until this specific check is done.
